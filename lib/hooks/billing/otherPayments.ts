...
export const addPayment = async (transaction: PaymentFormValues,user:any) => {
  try {
    ...
    await logAction(user.uid, role, "Billing", "payouts", 'add new E', { transactionAmount: transaction.paymentAmount,typeofPayment:transaction.typeofPayment,paymentDate:transaction.paymentDate,fromWho:transaction.fromWho});
